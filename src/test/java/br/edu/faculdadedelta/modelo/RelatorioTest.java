...
public class RelatorioTest extends BaseJPATest {
	...
	public void deveConsultarIdENomeConverterCliente() {
		...
		@SuppressWarnings("unchecked")
		List<Cliente> clientes = criteria.setResultTransformer(Transformers.aliasToBean(Cliente.class)).list();
	...
	public void deveConsultarVendaENomeClienteCasoExista() {
		...
		Criteria criteria = createCriteria(Venda.class, "v").createAlias("v.cliente", "c", JoinType.LEFT_OUTER_JOIN)
				.add(Restrictions.ilike("c.nome", "Flávio de Souza", MatchMode.START));

		@SuppressWarnings("unchecked")
		List<Venda> vendas = criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	...
	public void deveConsultarNotebooksSamsungOuDell() {
		...
		@SuppressWarnings("unchecked")
		List<Produto> notebooks = criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	...
	public void deveConsultarQuantidadeVendasPorCliente() {
		...
		Criteria criteria = createCriteria(Venda.class, "v").createAlias("v.cliente", "c")
	...
	public void deveConsultarDezPrimeirosProdutos() {
		...
		Criteria criteria = createCriteria(Produto.class, "p").setFirstResult(1).setMaxResults(10);
	...
	public void deveConsultarVendaPorNomeClienteUsandoSubquery() {
		...
		Criteria criteria = createCriteria(Venda.class, "v").createAlias("v.cliente", "cli")
	...
	public void deveConsultarTodosclientes() {
		...
		@SuppressWarnings("unchecked")
		List<Cliente> clientes = criteria.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY) // evida dados repetidos
				.list();
	...
	private void criarVendas(int quantidade) {
		...
		IntStream.range(0, quantidade).forEach(i -> {
			...
			venda.getProdutos().add(new Produto("Notebook", "Sony"));
